...
public class Parser {
	   ...
	   Type dims(Type p) throws IOException { //array -> array[num]
	      match('[');  Token tok = look;  match(Tag.NUM);  match(']'); //依次调用match匹配[num]
	      if( look.tag == '[' ) //如果是多维数组，递归调用
	      	p = dims(p);
	      return new Array(((Num)tok).value, p);
	   }

	   //过程stmt有一个switch语句，这个语句的各个case分支对应于非终止符号Stmt的各个产生式
	   //stmts -> stmts stmt | epslon
	   Stmt stmts() throws IOException { //stmts -> stmts stmt | epslon
		  // 因为前面一开始已经在block开头匹配过{了，才会进入非终止符号的匹配，所以此处直接匹配}，来判断是不是空语句 {}
	      ...
	      switch( look.tag ) {
				...
				case Tag.WHILE: //stmt->while(bool) stmt
		  		...
		  		case Tag.FOR:
					...
					match(Tag.FOR); match('('); s = stmt(); x = bool(); match(';'); s1 = stmt(); match(')');
					s2 = stmt();
	   ...
	   Expr bool() throws IOException { //bool不是单纯一个布尔变量，是一个表达式
	      Expr x = join(); //递归调用
